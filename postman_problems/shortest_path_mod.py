...
# Modified by:
#           Jack Connor <jconnor@baaqmd.gov>
"""
...
#JC arg 'pred=None' eliminated to default to 'pred={}' in _dijkstra_multisource
def _dijkstra(G, source, weight, paths=None, cutoff=None,
              target=None):
    ...
    """
    return _dijkstra_multisource(G, [source], weight, paths=paths,
                                 cutoff=cutoff, target=target)


#JC modify so that pred is also an argument for weight
def _dijkstra_multisource(G, sources, weight, pred={}, paths={},
    ...
    while fringe:
        ...
        for u, e in G_succ[v].items():
            #JC pred_node defined
            if v in pred:
                pred_node = pred[v][0]
            else:
                pred_node = None
              #JC pred_node argument added to weight
            cost = weight(v, u, e, pred_node)
            if cost is None:
                continue
            vu_dist = dist[v] + cost
